...
type RWS<'Env, 'State, 'Log, 'T> = RWS of ('Env -> 'State -> RWSResult<'State, 'Log, 'T>)
with
...
// Haskell Primitives
    ...
    val inline asks: func: (^e -> ^a) -> RWS< ^e, ^s, ^w, ^a>
        when ^w: (static member Zero: unit -> ^w)
...
// Monad
    ...
    module Workflow =
        ...
        type RWSBuilder =
            new: unit -> RWSBuilder
            member inline Return: x: ^a -> RWS< ^e, ^s, ^w, ^a> when ^w: (static member Zero: unit -> ^w)
            member ReturnFrom: m: RWS<'e, 's, 'w, 'a> -> RWS< ^e, ^s, ^w, ^a>
            member inline Zero: unit -> RWS< ^e, ^s, ^w, unit> when ^w: (static member Zero: unit -> ^w)
            member inline Bind: m: RWS< ^e, ^s, ^w, ^a> * f: (^a -> RWS< ^e, ^s, ^w, ^b>) -> RWS< ^e, ^s, ^w, ^b> when ^w: (static member ( + ): ^w -> ^w -> ^w)
