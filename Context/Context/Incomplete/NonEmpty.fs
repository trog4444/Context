namespace PTR.Context.Type//.Incomplete.NonEmpty
...
//  let singleton (x: 'a) : ^a NonEmpty = { NonEmpty.Head = x ; Tail = ULNil }

//  /// Add an item to the front of a `NonEmpty` sequence. / O(1) /
//  let cons (x: 'a) (xs: NonEmpty< ^a>) : NonEmpty< ^a> =
...
//      let wrap (x: 'a) : NonEmpty< ^a> = { NonEmpty.Head = x ; Tail = ULNil }
...
//        member inline _.ReturnFrom m : NonEmpty< ^a> = m
...
//      /// <exception cref="System.ArgumentNullException">Thrown when the input sequence is null.</exception>
//      let inline forA f (source: ^a seq) = sequenceA (System.Linq.Enumerable.Select(source, System.Func<_,_>f))
...
//      let inline ---END HERE--- f (source1: ^a seq) (source2: ^b seq) =
...
//      /// <exception cref="System.ArgumentNullException">Thrown when the input sequence is null.</exception>
//      let inline zipWithA f (source1: #seq< ^a>) (source2: #seq< ^b>) =
...
//      let extract (w: NonEmpty<'a>) : ^a = w.Head
