...
/// Standard operations on `Cont` values.
module Std =
...
/// Convert between values of type `Cont` and related types.
module Convert =

    /// Convert a 'Cont' value into a single-element sequence using 'id' as the continuation function.
    let inline toSeq (Cont c) = seq { yield c id }
