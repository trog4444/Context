...
// Minimal
    ...
    let inline local (localization: ^e -> ^e) (RWS r) : RWS< ^e, ^s, ^w, ^a> =
        RWS (fun e s -> r (localization e) s)
    ...
    let inline listens f (RWS r) : RWS< ^e, ^s, ^w, struct (^b * ^a)> =
        RWS (fun e s ->
            let res = r e s
            { RWSResult.State = res.State
            ; Log = res.Log
...
// Monad
    ...
    let inline fixM
        ...
        (em: Rogz.Context.Data.Either.Either< ^a, RWS< ^e, ^s, ^w, ^a>>) : RWS< ^e, ^s, ^w, ^b> =
        RWS (fun e s ->
            let mutable w = mt ()
            let mutable s = s
            let rec go (RWS r) =
                ...
                k res.Value
            and k a = loop k go a
            let res = Rogz.Context.Data.Either.Either.caseof k go em |> runRWS e s
            { res with
                RWSResult.State = s
                Log = w })
