...
[<Struct>]
type RWSResult<'state, 'log, 'value> = { State: ^state ; Log: ^log ; Value: ^value }
...
type RWSResult =
    ...
    static member inline OfTriple f : RWS< ^e, ^s0, ^s1, ^l, ^v> =
        RWS (fun e s -> match f e s with (s, l, v) -> { State = s ; Log = l ; Value = v })

    /// Convert a function yielding a pair into a State/Value function.
    static member inline OfTriple f : RWS< ^e, ^s0, ^s1, ^l, ^a> =
        RWS (fun e s -> match f e s with struct (s, l, a) -> { State = s ; Log = l ; Value = a })

    /// Create a ref-tuple from a state/log/value pair.
    static member inline ToTriple (r: RWSResult< ^s, ^l, ^v>) = (r.State, r.Log, r.Value)

    /// Create a struct-tuple from a state/log/value pair.
    static member inline ToTriple1 (r: RWSResult< ^s, ^l, ^v>) = struct (r.State, r.Log, r.Value)


/// Standard operations on `RWS` values.
module Std =
    ...
    let inline evalRWS (e: ^e) (s: ^s) (RWS r) : ^l * ^a  = match r e s with m -> m.Log, m.Value

    /// Evaluate a computation with the given initial state and environment,
    /// returning the final state and output, discarding the final value.
    let inline execRWS (e: ^e) (s: ^s) (RWS r) : ^``s*`` * ^l = match r e s with m -> m.State, m.Log
    ...
    let inline mapRWS (f: ^``s*`` -> ^l0 -> ^a0 -> RWSResult< ^``s**``, ^l, ^a>) (RWS r) =
        RWS (fun (e: ^e) (s: ^s) -> match r e s with m -> f (m.State: ^``s*``) m.Log m.Value)
...
module Composition =
    ...
    let inline wrap x : RWS< ^e, ^s, ^s, ^l, ^a> = RWS (fun _ s ->
        { State = s ; Log = (^l: (static member Empty: unit -> ^l) ()) ; Value = x })
    ...
    let inline bind (k: ^a -> RWS< ^e, ^``s*``, ^``s**``, ^l, ^b>) (RWS r) = RWS (fun e s ->
        ...
        | m -> match k m.Value with
               | RWS r -> match r e m.State with
                          | m2 -> { m2 with Log = (^l: (static member Append: ^l -> ^l -> ^l) (m.Log, m2.Log)) })
    ...
    let inline ap (RWS rv) (RWS rf) : RWS< ^e, ^s, ^``s**``, ^l, ^b> = RWS (fun e s ->
        ...
        | f -> match rv e (f.State: ^``s*``) with
               | v -> { State = v.State
                      ; Log = (^l: (static member Append: ^l -> ^l -> ^l) (f.Log, v.Log))
                      ; Value = f.Value v.Value })

    /// Lift a function onto effects.
    let inline map f (RWS r) : RWS< ^e, ^s, ^``s*``, ^l, ^b> = RWS (fun e s ->
        match r e s with
        | m -> { State = m.State ; Log = m.Log ; Value = f m.Value })
        ...
        module Zip =
            ...
            let inline munzip (RWS r) : RWS< ^e, ^s, ^``s*``, ^l, ^a> * RWS< ^e, ^s, ^``s*``, ^l, ^b> =
                RWS (fun e s -> match r e s with
                                | m -> { State = m.State ; Log = m.Log ; Value = fst (m.Value) }),
                RWS (fun e s -> match r e s with
                                | m -> { State = m.State ; Log = m.Log ; Value = snd (m.Value) })


    /// Creates a monadic workflow for the given type.
    let rws = Monad.RWSBuilder ()
