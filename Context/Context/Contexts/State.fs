...
type StateValue =
    ...
    static member inline ToPair { State = state ; Value = value } = (state, value)

    /// Create a struct-tuple from a state/value pair.
    static member inline ToPair1 { State = state ; Value = value } = struct (state, value)


/// Standard operations on `State` values.
module Std = 
...
module Composition =
    ...
    module Applicative =
        ...
        let inline replicateA (n: uint32) m : State< ^s, ^s, ^a seq> =                
            State (fun s0 ->
                let (State f) = Std.cacheState m
                let mutable st = s0
