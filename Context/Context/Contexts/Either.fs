...
// Monad    
    ...
    let inline fixM loop (em: Either< ^a, Either< ^e, ^a>>) : Either< ^e, ^b> =
        ...
        and k a = loop k go a
        match em with Left a -> k a | Right m -> go m
    ...
    module Workflow =
        ...
        type EitherBuilder() =
            member inline _.Return(x) : Either< ^e, ^a> = Right x
            member inline _.ReturnFrom(m) : Either< ^e, ^a> = m
            member inline _.Bind(m: Either< ^e, ^a>, f) : Either< ^e, ^b> = bind f m
            member inline _.Zero() : Either< ^e, unit> = Right ()
