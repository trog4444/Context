...
//            let inline replicateA (_: int) (_: Proxy< ^a>) : Proxy< ^a seq> = PROXY
...
//            let replace (_: ^b) (_: Proxy< ^a>) : Proxy< ^b> = PROXY
...
//            let mempty<'a> : Proxy< ^a> = PROXY

//            /// Repeat a value 'n' times.
//            let inline mtimes (_: int) (_: Proxy< ^a>) : Proxy< ^a> = PROXY
...
//            let identity<'a> : Proxy< ^a, ^a> = PROXY2
...
//                let app<'a, 'b> : Proxy<Proxy<'a, 'b> * 'a, 'b> = PROXY2


//    /// Creates a computation expression for the given type.
//    let proxy = Compose.Monad.ProxyBuilder ()
